...
from model import Session, Usuario, Livro, Endereco
...
# definindo tags
endereco_tag = Tag(name="Endereco", description="Adicionar e consultar endereço do usuário")
...
def add_endereco(form: EnderecoSchema):
    ...
    url = 'https://viacep.com.br/ws/' + form.cep + '/json/'
    responseViaCEP = requests.get(url).json()

    endereco = Endereco(
        cep = form.cep,
        logradouro = responseViaCEP["logradouro"],
        bairro = responseViaCEP["bairro"],
        uf = responseViaCEP["uf"],
        numero = form.numero,
        complemento = form.complemento,
        endereco_de = form.endereco_de
    )
    ...
    try:
        ...
        session.commit()
        logger.debug(f"Adicionado o endereco com o cep: '{endereco.cep}'")
        return '', 200
    except Exception as e:
        # caso um erro fora do previsto
        #
        error_msg = "Não foi possível adicionar o endereço :/"
        logger.warning( f"Erro adicionar endereço, {error_msg}")
        return {"mensagem": error_msg}, 404

        # error_msg = "Não foi possível salvar novo item :/"
        # error_msg = e
        # logger.warning(f"Erro ao adicionar o endereço de cep '{endereco.cep}', {error_msg}")
        # print()
        # return {"mensagem": error_msg}, 400

@app.get('/enderecos', tags=[endereco_tag],
         responses={"200": ListagemEnderecosSchema, "404": ErrorSchema})
def get_enderecos():
    """Faz a busca por todos os Enderecos cadastrados

    Retorna uma representação da listagem dos enderecos.
    """
    logger.debug(f"Coletando enderecos ")
    # criando conexão com a base
    session = Session()
    # fazendo a busca
    ...
    if not enderecos:
        # se não há endereços cadastrados
        return {"enderecos": "Não há enderecos cadastrados"}, 200
    else:
...
def add_usuario(form: UsuarioSchema):
    ...
    print("TENTANDO ADICIONAR USUARIO")
    usuario = Usuario(
        nome=form.nome,
        idade=form.idade)
